...
/* Includes ---------------------------------------------------------------- */
#include <Camera_trap_inferencing.h>
...
const int SendDataPin = 12;

// Define Output Parameters
int MLClassId = 0;          // ML Class Id (use numbers 0-255)
int MLPredictionValue = 0;  // ML Prediction in %
int Reserved = 0;          // Reserved (2B)
boolean DoSendPacket = 1;   // Send Packet to LoRaWAN
int SleepMinutes = 1;       // Turn Off Arduino for 1 Minutes
...
void setup()
{
    // Turn Off Power and Buildin Led to reduce power consumption
    ...
    // Notify BeatML that Nano is ready
    digitalWrite(ReadyPin, HIGH);
    digitalWrite(SendDataPin, LOW);
    
    // Initialize Serial1 for connection to BeatML
    Serial1.begin(9600);
  
    // Wait for Arduino to stabilize GPIO channels
    delay(500);
}
...
#else
        MLClassId = 0;
        MLPredictionValue = int(100*(result.classification[0].value));
        for (size_t ix = 1; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
            if(int(100*(result.classification[ix].value)) > MLPredictionValue) {
              MLClassId = ix;
              MLPredictionValue = int(100*(result.classification[ix].value));
            }
        }
        Data2BeatML(MLClassId, MLPredictionValue, Reserved, DoSendPacket, SleepMinutes);
#endif
...
void Data2BeatML(int iMLClassId, int iMLPredictionValue, int iReserved, int iDoSendPacket, int iSleepMinutes) {

  // Split int to 2B
  int iReservedMSB = floor(iReserved/256);
  int iReservedLSB = iReserved - iReservedMSB;   
  int iSleepMinutesMSB = floor(iSleepMinutes/256);
  int iSleepMinutesLSB = iSleepMinutes - iSleepMinutesMSB;  

  // Prepare the Data Packet
  byte dataPacket[7];
  dataPacket[0] = (char)iMLClassId;
  dataPacket[1] = (char)iMLPredictionValue;
  dataPacket[2] = (char)iReservedMSB;
  dataPacket[3] = (char)iReservedLSB;
  dataPacket[4] = (char)iDoSendPacket;
  dataPacket[5] = (char)iSleepMinutesMSB;
  dataPacket[6] = (char)iSleepMinutesLSB; 

  // Notify BeatML of the packet start
  digitalWrite(SendDataPin, HIGH);

  // Send Data to Serial1
  delay(5);
  Serial1.write(dataPacket, 7);
  delay(5);

  // Notify BeatML of the packet end
  digitalWrite(SendDataPin, LOW);
}